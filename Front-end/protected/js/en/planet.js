...
fetch('http://localhost:8000/api/admin/planets', {
...
.then(data => {
    data.forEach(item => {
        ...
        pFrName.classList.add('Edit--planets--container--element--fr_name');
        pFrName.textContent = item.fr_name;

        let pEnName = document.createElement('p');
        pEnName.classList.add('Edit--planets--container--element--element');
        pEnName.classList.add('Edit--planets--container--element--en_name');
        pEnName.textContent = item.en_name;
        ...
        pDistance.classList.add('Edit--planets--container--element--distance');
        pDistance.textContent = item.distance;

        let pTime = document.createElement('p');
        pTime.classList.add('Edit--planets--container--element--element');
        pTime.classList.add('Edit--planets--container--element--distance');
        ...
        aShow.addEventListener('click', function(){
            ...
            window.location.href = '../../en/destination.html';
        })
        ...
        aEdit.href = "edit_planet.html?id=" + item.id;
        ...
        bDestroy.addEventListener('click', function(e){
            e.preventDefault(); 
            fetch('http://localhost:8000/api/admin/planet/' + item.id + '/destroy', {
                method: 'GET',
                headers: {
                    'Authorization': getCookieValue('token'),
                }
            ...
            .then(data => {
                console.log(data)
                create_message.textContent = "Planet deleted successfully";
                li.remove();
            })
            .catch(error => {
                create_message.textContent = "An error occurred while deleting the planet";
                console.error(error)
            });
        });
