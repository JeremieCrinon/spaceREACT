...
fetch(`http://localhost:8000/api/tech/${id}`)
    ...
    .then(data => {
        ...
        planet_img.src = "http://localhost:8000/api/techImg/" + imagePath;
...
edit_planet_form.addEventListener('submit', function(e){
    ...
    fetch(`http://localhost:8000/api/admin/tech/${id}/edit`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        console.log(data)
        sessionStorage.setItem('message', "Tech edited successfully");
        window.location.href = 'tech.html';
    })
    .catch(error => {
        console.error(error);
        sessionStorage.setItem('message', "A problem occurred while editing the tech");
        window.location.href = 'tech.html';
    });
});