...
fetch('http://localhost:8000/api/admin/teches', {
...
.then(data => {
    data.forEach(item => {
        ...
        aEdit.href = "edit_tech.html?id=" + item.id;
        ...
        bDestroy.addEventListener('click', function(e){
            e.preventDefault(); 
            fetch('http://localhost:8000/api/admin/tech/' + item.id + '/destroy', {
                method: 'GET',
                headers: {
                    'Authorization': getCookieValue('token'),
                }
            ...
            .then(data => {
                console.log(data)
                create_message.textContent = "Tech deleted successfully";
                li.remove();
            })
            .catch(error => {
